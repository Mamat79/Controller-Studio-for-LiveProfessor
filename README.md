...
La version 2026.1 réutilise d'abord les affectations actives et les presets manuels existants, puis construit un profil unique partagé par toutes les instances d'un même type de plugin. Le choix majoritaire est conservé en cas de divergence, tandis qu'une modification manuelle active reste prioritaire sur un ancien preset. Les fonctions manuelles complémentaires sont conservées dans les rotatifs encore libres, dans la limite de la banque choisie. Une fonction déjà présente n'est pas automatiquement rajoutée ailleurs. La seule répétition volontaire est le couple `poussoir N + rotatif N` avec le même paramètre : elle permet au label du poussoir de rester visible en permanence sur l'EC4. Les raccourcis Shift ne sont jamais modifiés.

L'affichage suit une règle simple : un rotatif mappé affiche son label ; si le rotatif est vide mais que son poussoir est mappé, la case affiche le label du poussoir ; si les deux existent, le label rotatif reste prioritaire. Lors de l'appui, le bridge affiche le nom et la valeur renvoyés pour le poussoir lorsque LiveProfessor les fournit.
...
Version 2026.1 first reuses active assignments and existing manual presets, then builds one shared profile for every instance of the same plugin type. The majority choice wins when instances differ, while an active manual edit remains authoritative over an older preset. Complementary manual functions are retained on remaining free rotaries, within the capacity of the selected bank. A function already present is not automatically mapped somewhere else. The only intentional duplicate is a matched `push N + rotary N` pair carrying the same parameter, which keeps the push label permanently visible on the EC4. Shift shortcuts are never changed.

Display priority is straightforward: a mapped rotary shows its label; when the rotary is empty but its push is mapped, the cell shows the push label; when both exist, the rotary label remains visible. On press, the bridge displays the push name and value when LiveProfessor provides them.
