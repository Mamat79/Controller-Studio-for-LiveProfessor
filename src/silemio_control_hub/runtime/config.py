"""Persistent real-time controller configuration for SiLeMI/O."""
...
from ..platform_paths import previous_product_data_dir, product_data_dir


@dataclass(slots=True)
class BridgeConfig:
    """Configuration persistante du moteur EC4/LiveProfessor.
...
def legacy_config_path() -> Path:
    ...
    if base:
        return Path(base) / "EC4LiveProfessorBridge" / "config.json"
    return Path.home() / ".ec4-liveprofessor-bridge" / "config.json"
...
def default_config_path() -> Path:
    portable = executable_dir() / "silemio-runtime.json"
    if portable.exists():
        return portable
    return default_data_dir() / "runtime-config.json"


def previous_product_config_path() -> Path:
    return previous_product_data_dir() / "runtime-config.json"


def load_config(path: Path | None = None) -> BridgeConfig:
    explicit_path = path is not None
    path = path or default_config_path()
    if not explicit_path and not path.exists():
        for candidate in (previous_product_config_path(), legacy_config_path()):
            if candidate.exists():
                path = candidate
                break
    if not path.exists():
