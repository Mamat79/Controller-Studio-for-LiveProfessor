"""Lecteur/écrivain binaire JUCE ValueTree minimal appartenant à SiLeMI/O."""
...
class _Reader:
    ...
    def read_tree(self) -> ValueTree:
        ...
        properties = [
            (self.read_string(), self.read_variant()) for _ in range(property_count)
        ]
...
def _walk(tree: ValueTree) -> Iterator[ValueTree]:
    yield tree
    for child in tree.children:
...
def normalize_rotary_controls(tree: ValueTree, rotary_count: int) -> dict[str, int]:
    """Resize a Companion controller rotary list while preserving its structure."""

    if not 1 <= rotary_count <= 99:
        raise ValueTreeFormatError("le nombre de rotatifs doit etre compris entre 1 et 99")
    ...
    for control in controls_node.children:
        ...
        if match and control.type_name == "HardwareControl" and control.get("ControlStyle") == 0:
            number = int(match.group(1))
            if number in rotaries:
                raise ValueTreeFormatError(f"rotatif {number} defini plusieurs fois")
            rotaries[number] = control

    if 1 not in rotaries:
        raise ValueTreeFormatError("le rotatif de base 1 est absent")

    removed_ids: set[int] = set()
    kept_children: list[ValueTree] = []
    removed = 0
    ...
    template = rotaries[max(rotaries)]
    added = 0
    for number in range(1, rotary_count + 1):
        if number in rotaries:
            continue
